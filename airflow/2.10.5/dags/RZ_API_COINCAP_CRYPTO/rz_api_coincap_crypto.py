...
def gerar_sql_insert_coincap(**context):
    ...
    url = f"https://api.coincap.io/v2/assets?ids={','.join(ids)}&limit=2000"
    ...
    for item in data:
        linha = "STRUCT(" + ", ".join([
            f"{escape_sql_string(item.get('id'))} AS id",
            f"{escape_sql_string(item.get('rank'))} AS rank",
            f"{escape_sql_string(item.get('symbol'))} AS symbol",
            f"{escape_sql_string(item.get('name'))} AS name",
            f"{escape_sql_string(item.get('supply'))} AS supply",
            f"{escape_sql_string(item.get('maxSupply'))} AS maxSupply",
            f"{escape_sql_string(item.get('marketCapUsd'))} AS marketCapUsd",
            f"{escape_sql_string(item.get('volumeUsd24Hr'))} AS volumeUsd24Hr",
            f"{escape_sql_string(item.get('priceUsd'))} AS priceUsd",
            f"{escape_sql_string(item.get('changePercent24Hr'))} AS changePercent24Hr",
            f"{escape_sql_string(item.get('vwap24Hr'))} AS vwap24Hr",
            f"{escape_sql_string(item.get('explorer'))} AS explorer"
        ]) + ")"
...
with DAG(
    dag_id="rz_api_coincap_crypto",
    description="Carrega dados da CoinCap Assets direto pro BigQuery",
    default_args=default_args,
    schedule_interval="*/30 * * * *",
    catchup=False,
    tags=["coincap", "bigquery", "etl", "assets", "rz"],
) as dag:
    ...
    acionar_dag_sz = TriggerDagRunOperator(
        ...
        trigger_dag_id="sz_api_coincap_crypto",
